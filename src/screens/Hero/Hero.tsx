...
console.log(errors)
  ...
  const handleSignedUp = async () => {
    ...
    if (!result.success) {
      ...
      toast.error("براہ کرم تمام فیلڈز کو صحیح طریقے سے پُر کریں۔");
      setIsLoading(false);
      return;
    }
    ...
    // Submit to Firebase
    try {
      await addDoc(collection(db, "waitlistUrdu"), formData);
      toast.success(
        "انتظار کی فہرست میں شامل ہونے کا شکریہ! ہم جلد ہی آپ تک پہنچیں گے۔",
        {
          duration: 4000,
          position: "top-center",
        }
      ...
      toast.error("ڈیٹا شامل کرنے میں خرابی۔ براہ کرم دوبارہ کوشش کریں۔");
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="absolute  top-0 left-0 flex flex-col  items-center gap-12 justify-center w-full h-full  text-textSecondary">
        <h1 className="text-3xl md:text-5xl text-center font-semibold text-white capitalize leading-snug tracking-wider">
          <ReactTyped
            ...
            strings={[ "پیاروں کو پیسے بھیج رہے ہیں؟","مستقبل کے لیے بچت؟"]}
          />
        </h1>
        <div className="flex flex-col gap-4 trext-center justify-center items-center">
          <p className="text-lg md:text-xl text-gray-300">ہم آپ کی مدد کر سکتے ہیں۔</p>
          <p className="text-lg md:text-xl text-gray-300">
          سادہ آسان قابل اعتماد{" "}
            <span className="text-white font-semibold">کوئی پوشیدہ فیس نہیں۔</span>
          </p>
          <p className="text-lg md:text-xl text-gray-300">
            <span className="text- font-semibold">Vision X</span>
          </p>
          <p className="text-lg md:text-xl font-medium text-gray-200">
          میں شامل ہوں۔{" "}
            <span className="text-indigo-500 font-bold">وی آئی پی ویٹنگ لسٹ</span>.
          </p>
        </div>
